...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $_SESSION['experiment'] = $assignmentTableName;
    $_SESSION['midsem'] = $submissionTableName;
...
$createTableSQL2 .= ")";
            ...
            if ($numSubdivisions > 1) {
                for ($s = 1; $s <= $numSubdivisions; $s++) {
                    ...
                    if(isset($_POST['num_mid_sem'])) {
                        $_SESSION['num_mid_sem'] = $_POST['num_mid_sem']; // Store num_mid_sem in the session
                    }
                    ...
                    $insertSQL = "INSERT INTO $submissionTableName (mid_sem, question, subdivision_mark, subdivision_category) VALUES (?, ?, ?, ?)";
    ...
    for ($i = 1; $i <= $num_experiments; $i++) {
        ...
        $insertSQL = "INSERT INTO $assignmentTableName (experiment_no, ";
        for ($k = 1; $k <= $co_no; $k++) {
            $insertSQL .= "CO$k";
