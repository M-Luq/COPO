...
<body>
  ...
  <div class="container">
  <form  method="post" action = "../labheaders/navigate3.php">
    ...
    if ($result_assessments->num_rows > 0) {
        while($assessment_row = $result_assessments->fetch_assoc()) {
            ...
            $assessment = $assessment_row["mid_sem"];
            $_SESSION['mid_sem'][] = (object) array('name' => $assessment);
            
            echo '<h2>Mid sem ' . $assessment . '</h2>';
...
$sql_questions = "SELECT id, question, subdivision_mark, subdivision_category FROM $submissionTableName WHERE mid_sem = $assessment";
...
if ($result->num_rows > 0) {
    ...
    $sql_assignments = "SELECT $selectColumns FROM $assignmentTableName WHERE experiment_no = $i";
...
if ($result_assignments->num_rows > 0) {
  while($row = $result_assignments->fetch_assoc()) {
    ...
    for ($co = 1; $co <= $co_no; $co++) {
      ...
      echo "No experimentqn found for this student.";
      }
          }
