...
$sheet->fromArray([$MM], null, 'A4');
    ...
    for ($assessment = 1; $assessment <= $num_assessments; $assessment++) {
        for ($coNumber = 1; $coNumber <= $co_no; $coNumber++) {
            ...
            $query = "SELECT question FROM $submissionTableName WHERE subdivision_category = '$coCategory' AND mid_sem = $assessment";
            try {
    ...
    for ($assessment = 1; $assessment <= $num_assessments; $assessment++) {
        ...
        $query = "SELECT subdivision_mark FROM $submissionTableName WHERE subdivision_category = '$coCategory' AND mid_sem = $assessment";
        try {
            $stmt = $dbh->prepare($query);
            $stmt->execute();
...
*/    
    ...
    for ($assignmentNumber = 1; $assignmentNumber <= $num_assignments; $assignmentNumber++) {
        ...
        $queryAssignment = "SELECT * FROM $assignmentTableName WHERE experiment_no = $assignmentNumber";
        try {
...
for ($j = 1; $j <= $num_assessments; $j++) {
    $sql = "SELECT * FROM $uniqueid WHERE midsem_no = $j";
...
for ($j = 1; $j <= $num_assignments; $j++) {
    $sql = "SELECT * FROM $uniqueid2 where experiment_no =$j";
    $stmt = $dbh->prepare($sql);
    $stmt->execute();
