...
$submissionTableName = $_SESSION['midsem'];
$assignmentTableName = $_SESSION['experiment'];
...
$sql_assessments = "SELECT DISTINCT mid_sem FROM $submissionTableName";
...
foreach ($assessmentNames as $assessment) {
    ...
    foreach ($studentNames as $studentRegNo => $studentName) {
        $insertRowSQL = "INSERT INTO $uniqueId (midsem_no, student_regno, ";
...
$sql_assignments = "SELECT DISTINCT experiment_no FROM $assignmentTableName";
$result_assignments = $conn->query($sql_assignments);
$assignmentNames = array();
if ($result_assignments->num_rows > 0) {
    while ($row = $result_assignments->fetch_assoc()) {
        $assignmentNames[] = $row['experiment_no'];
    }
}
...
$createTableSQL = "CREATE TABLE $uniqueId2 (id INT AUTO_INCREMENT PRIMARY KEY,experiment_no INT, student_regno VARCHAR(255), ";
...
foreach ($assignmentNames as $assignment) {
    ...
    foreach ($submittedStudentMarks as $studentRegNo => $studentAssignmentMarks) {
        $insertRowSQL = "INSERT INTO $uniqueId2 (experiment_no, student_regno";
...
for ($assessmentNumber = 1; $assessmentNumber <= $n; $assessmentNumber++) {
    echo '<h2 class="table-title">Mid sem ' . $assessmentNumber . '</h2>';
    echo '<table border="1">';
    echo '<thead>';
    echo '<tr>';
    echo '<th>SINO</th>';
    ...
    echo '<tbody>';
        ...
        $sql .= " FROM $uniqueId WHERE midsem_no = $assessmentNumber";
...
for ($assignmentNumber = 1; $assignmentNumber <= $n; $assignmentNumber++) {
    echo '<h2 class="table-title">Experiment ' . $assignmentNumber . '</h2>';
    echo '<table border="1">';
    echo '<thead>';
    echo '<tr>';
    echo '<th>SINO</th>';
    ...
    echo '<tbody>';
        ...
        $sql .= " FROM $uniqueId2 WHERE experiment_no = $assignmentNumber";
