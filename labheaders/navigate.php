...
if (isset($_FILES["excel_file_l"]) && $_FILES["excel_file_l"]["error"] == 0) {
    // Define the target directory where uploaded files will be stored
    $uploadDir = __DIR__.'/../uploads/';

    // Get the uploaded file's name and path
    $uploadFilePath = $uploadDir . basename($_FILES["excel_file_l"]["name"]);

    // Move the uploaded file to the target directory
    if (move_uploaded_file($_FILES["excel_file_l"]["tmp_name"], $uploadFilePath)) {
        try {
            // Load the Excel file
            ...
            $tableName = pathinfo($_FILES["excel_file_l"]["name"], PATHINFO_FILENAME);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['num_mid_sem'])) {
        $_SESSION['num_mid_sem'] = $_POST['num_mid_sem'];
    }
    
    if (isset($_POST['num_experiments'])) {
        $_SESSION['num_experiments'] = $_POST['num_experiments'];
    }
    if (isset($_POST['subject_code'])) {
        $_SESSION['subject_code'] = $_POST['subject_code'];
    }

echo $_SESSION['num_mid_sem'];
...
header("Location: ../lab/pattern.php");
